...
_LEFT_W  = 252  # largo o bastante para a régua de comandos caber
_RIGHT_W = 340
...
class NeuralScene(QWidget if not _WEBENGINE else QWebEngineView):
    ...
    def __init__(self, parent=None):
        ...
        # Sem isto o QtWebEngine barra uma página file:// de buscar o CDN do
        # three.js (LocalContentCanAccessRemoteUrls vem desligado).
        s = self.settings()
        s.setAttribute(QWebEngineSettings.WebAttribute.LocalContentCanAccessRemoteUrls, True)
        s.setAttribute(QWebEngineSettings.WebAttribute.LocalContentCanAccessFileUrls, True)
        s.setAttribute(QWebEngineSettings.WebAttribute.WebGLEnabled, True)
        pagina = Path(__file__).resolve().parent / "scene" / "neural.html"
        self.loadFinished.connect(self._ao_carregar)
        self.load(QUrl.fromLocalFile(str(pagina)))

    def _ao_carregar(self, ok: bool):
        self._pronto = bool(ok)
...
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        self._draw_crt_overlay(p, W, H)

    def _draw_crt_overlay(self, p: QPainter, W: int, H: int):
        """Scanlines + vinheta por cima de tudo — a assinatura visual do
        style.txt do canal (repeating-linear-gradient + radial-gradient)."""
        p.setPen(QPen(qcol("#000000", 38), 1))
        for y in range(0, H, 2):
            p.drawLine(0, y, W, y)

        vig = QRadialGradient(W / 2, H / 2, max(W, H) * 0.72)
        vig.setColorAt(0.40, qcol("#000000", 0))
        vig.setColorAt(0.75, qcol("#05000c", 110))
        vig.setColorAt(1.00, qcol("#000000", 216))
...
class LogWidget(QTextEdit):
    ...
    def _next(self):
        ...
        elif tl.startswith("alpha:"): self._tag = "ai"
...
class ViewerPanel(QWidget):
    ...
    def __init__(self):
        ...
        self._back = QPushButton("◄  HUD")
        self._back.setCursor(Qt.CursorShape.PointingHandCursor)
        self._back.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._back.setStyleSheet(
            f"QPushButton {{ color: {C.PRI}; background: {C.PANEL}; "
            f"border: 1px solid {C.BORDER_B}; padding: 4px 12px; }} "
        ...
        self._doc.setFont(QFont("Consolas", 10))
        self._doc.setStyleSheet(
            f"QTextEdit {{ color: {C.TEXT}; background: {C.PANEL}; "
            f"border: 1px solid {C.BORDER}; padding: 10px; }}"
        )
        lay.addWidget(self._doc, stretch=1)
        ...
        self._play_btn = QPushButton("▶  REPRODUZIR")
        for b, slot in ((self._play_btn, self._toggle_play),):
            b.setCursor(Qt.CursorShape.PointingHandCursor)
            b.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
            b.setStyleSheet(
                f"QPushButton {{ color: {C.GREEN}; background: {C.PANEL}; "
                f"border: 1px solid {C.GREEN_D}; padding: 5px 14px; }} "
                f"QPushButton:hover {{ background: #001a10; }}"
            )
            b.clicked.connect(slot)
        self._controls.addWidget(self._play_btn)
        self._controls.addStretch(1)
        self._ctrl_host = QWidget()
        self._ctrl_host.setLayout(self._controls)
        self._ctrl_host.hide()
        lay.addWidget(self._ctrl_host)
    ...
    def _toggle_play(self):
        if self._player.playbackState() == QMediaPlayer.PlaybackState.PlayingState:
            self._player.pause()
            self._play_btn.setText("▶  REPRODUZIR")
        else:
            self._player.play()
    ...
    def show_document(self, title: str, markdown_text: str):
        ...
        # setMarkdown dá títulos/listas/negrito de graça — os arquivos das
        # skills já vêm em markdown.
        if markdown_text.strip():
            self._doc.setMarkdown(markdown_text)
        else:
            self._doc.setPlainText("(vazio — nada gerado ainda)")
        self._doc.show()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        # Centro: núcleo 3D, HUD clássico e visualizador dividem o espaço.
        # O núcleo é o padrão; o HudCanvas fica como reserva (e continua
        # guardando os estados usados pelo resto da janela).
        self.hud = HudCanvas(face_path)
        self.hud.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        self.nucleo = NeuralScene()
        self.nucleo.setSizePolicy(
        ...
        self._center.addWidget(self.nucleo)
        self._center.addWidget(self.hud)
        self._center.addWidget(self.viewer)
        self.viewer.back_button.clicked.connect(self.show_hud)
        body.addWidget(self._center, stretch=5)
        ...
        # Sinais: as ferramentas rodam em threads; a UI só pode ser tocada
    ...
    def _build_header(self) -> QWidget:
        ...
        mid.addWidget(title)
        sub = QLabel("Assistente Lógico de Produção, Histórias e Automação")
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        lay.addSpacing(8)
        lay.addWidget(self._build_commands_panel())
        lay.addStretch()
        ...
        return w
    # Régua de comandos locais — os mesmos que `tools/local_commands.py`
    # reconhece. Fica sempre à vista para não precisar decorar; `<nome>` é a
    ...
    _COMANDOS = [
        ...
        ("Ctrl+Espaço", "trava o micro (ele não corta)"),
        ("— conteúdo —", None),
        ...
        ("modo conversa", "voz em tempo real (gasta cota)"),
        ("modo econômico", "volta ao local (poupa cota)"),
        ...
        ("apagar a aula X", "descarta uma gravação"),
        ("assistir o curso inteiro", "eu vejo e gravo aula por aula"),
        ("como está o curso", "em que aula eu estou"),
        ("parar o curso", "encerra a maratona"),
        ("processar curso", "transcreve e tira as regras"),
        ("revisar regras", "aprova o que vai valer"),
        ("— as seis fases —", None),
        ("fase 0 da <criatura>", "pesquisa; 1 model sheets, 2 storyboards"),
        ("pode seguir", "avança de onde parou"),
        ("em que pé estamos", "a fase de cada criatura"),
        ("— produção —", None),
        ...
        ("andamento", "como vai"),
        ("— redes —", None),
        ("login <rede>", "você entra; sem senha aqui"),
        ("postar <nome> no <rede>", "prepara, não publica"),
        ("— apagar —", None),
    ...
    def _build_commands_panel(self) -> QWidget:
        ...
        for comando, descricao in self._COMANDOS:
            if descricao is None:
                sec = QLabel(comando)
                sec.setFont(QFont("Courier New", 6))
                sec.setStyleSheet(
            ...
            lh = QHBoxLayout(linha)
            lh.setContentsMargins(0, 0, 0, 0)
            lh.setSpacing(4)

            cmd = QLabel(comando)
            cmd.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
            cmd.setStyleSheet(f"color: {C.ACC2}; background: transparent; border: none;")
            lh.addWidget(cmd)

            desc = QLabel(descricao)
            desc.setFont(QFont("Courier New", 6))
            desc.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
            desc.setAlignment(
                Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            )
            lh.addWidget(desc, stretch=1)

            lay.addWidget(linha)

        rodape = QLabel("qualquer outra frase vai para a voz")
        rodape.setFont(QFont("Courier New", 6))
        rodape.setWordWrap(True)
        rodape.setStyleSheet(
            f"color: {C.TEXT_DIM}; background: transparent; border: none; padding-top: 5px;"
        )
    ...
    def _toggle_mute(self):
        ...
        self.nucleo.set_state("MUTED" if self._muted else "LISTENING", self._muted)
        self._style_mute_btn()
        if self._muted:
    ...
    def _apply_state(self, state: str):
        ...
        self.nucleo.set_state(state, self._muted)

    def _show_document(self, title: str, text: str):
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            return bool(d.get("openai_api_key"))
        except Exception:
            return False
...
class OmegaUI:
    ...
    def show_document(self, title: str, markdown_text: str):
        self._win._doc_sig.emit(title, markdown_text)
